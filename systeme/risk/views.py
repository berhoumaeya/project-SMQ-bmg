...
# Afficher tous les Risk

@method_decorator(login_required(login_url='login'), name='dispatch')
class DashboardRiskAPIView(APIView):
...
# Ajouter un Risk
@method_decorator(login_required(login_url='login'), name='dispatch')
class CreateRiskAPIView(APIView):
...
# Modifier un Risk
@method_decorator(login_required(login_url='login'), name='dispatch')
class UpdateRiskAPIView(APIView):
    permission_classes = [IsAuthenticated]
    
    def put(self, request, pk):
        risk = get_object_or_404(Risque, pk=pk)
        serializer = RisqueSerializer(risk, data=request.data)
        if serializer.is_valid():
...
# Afficher un Risk
@method_decorator(login_required(login_url='login'), name='dispatch')
class SingularRiskAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def get(self, request, pk):
        risk = get_object_or_404(Risque, pk=pk)
        serializer = RisqueSerializer(risk)
...
# Supprimer un Risk
@method_decorator(login_required(login_url='login'), name='dispatch')
class DeleteRiskAPIView(APIView):
    permission_classes = [IsAuthenticated]

    def delete(self, request, pk):
        fiche_risk = get_object_or_404(Risque, pk=pk)
        fiche_risk.delete()
