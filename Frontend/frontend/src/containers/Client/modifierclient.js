...
function ModifierClient() {
    ...
    useEffect(() => {
        const fetchclient = async () => {
            try {
              ...
              setnom(data.nom);
              setcode_client(data.code_client);
              setraison_sociale(data.raison_sociale);
              setactivite(data.activite)
              settype_client(data.type_client);
    ...
    const handleSubmit = (event) => {
        ...
        axios.put(`${process.env.REACT_APP_API_URL}/CRM/update_client/${id}/`, formData, { headers })
            .then(response => {
    ...
    if (updateReussi) {
        return <Navigate to={`/ConsulterClient/${id}/`}/>;
    }
            ...
            <div className="form-card">
                ...
                <div className="form-group">
                    ...
                    <div className="form-group">
                        ...
                        {piecesJointesUrl ? (
                            <div>
                                <input type="text" value={piecesJointesUrl} onChange={(e) => setPiecesJointesUrl(e.target.value)} />
                            </div>
                        ) : (
                            <input 
                                type="file" 
                    ...
                    <div className="button-group">
                        ...
                        <Link to={`/ConsulterClient/${id}/`} className="btn btn-gray mt-3">Annuler</Link>
                    </div>
                </form>
            </div>
        </div>
