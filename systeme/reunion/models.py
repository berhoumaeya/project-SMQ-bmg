...
class Meeting(models.Model):
    ...
    participants = models.ManyToManyField(User, related_name='meetings_attended',limit_choices_to={'groups__name': 'Participant'})
    piece_jointe = models.FileField(upload_to='meeting_attachments/', blank=True, null=True)
    commentaire = models.TextField(blank=True, null=True)

    def __str__(self):
        return f"Réunion demandée par {self.demandeur.username} le {self.date_previsionnelle}"
